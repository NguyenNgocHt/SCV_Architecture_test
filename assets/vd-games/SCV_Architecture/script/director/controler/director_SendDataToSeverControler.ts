...
export class director_SendDataToSeverControler {
  ...
  registerEventFromLoginScreen() {
    ...
    VDEventListener.on(GAME_EVENT_DEFINE.HOME_SEND_PLAYER_ID_TO_DIRECTOR, this.sendDataToSever_FromPlayerInHome.bind(this));
  }
  offEvent() {
    VDEventListener.off(GAME_EVENT_DEFINE.SEND_DATA_TO_DIRECTOR_FROM_LOGIN, this.sendDataToSever_fromLogin.bind(this));
    VDEventListener.off(GAME_EVENT_DEFINE.HOME_SEND_PLAYER_ID_TO_DIRECTOR, this.sendDataToSever_FromPlayerInHome.bind(this));
  }
  //from login
  ...
  sendDataToSever_FromPlayerInHome(data) {
    VDEventListener.dispatchEvent(GAME_EVENT_DEFINE.SEND_PLAYER_IN_HOME_NODE_DATA_FROM_DIRECTOR, data);
  }
}
