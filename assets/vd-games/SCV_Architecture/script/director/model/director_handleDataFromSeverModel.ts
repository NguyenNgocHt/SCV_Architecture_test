...
export class director_handleDataFromSeverModel {
  ...
  registerEvent() {
    VDEventListener.on(GAME_EVENT_DEFINE.SEND_LOGIN_RESULT_DATA_TO_DIRECTOR, this.handleLoginResultData.bind(this));
    VDEventListener.on(GAME_EVENT_DEFINE.SEND_PLAYER_INFO_TO_DIRECTOR, this.handlePlayerInfo.bind(this));
  }
  offEvent() {
    VDEventListener.off(GAME_EVENT_DEFINE.SEND_LOGIN_RESULT_DATA_TO_DIRECTOR, this.handleLoginResultData.bind(this));
    VDEventListener.off(GAME_EVENT_DEFINE.SEND_PLAYER_INFO_TO_DIRECTOR, this.handlePlayerInfo.bind(this));
  }
  handleLoginResultData(loginResult: loginResult) {
    console.log("come in director_handleDataFromSeverModel");
    VDEventListener.dispatchEvent(GAME_EVENT_DEFINE.SEND_LOGIN_RESULT_DATA_TO_SCREENS_CONTROLER, loginResult);
    this.setLoginResultData(loginResult);
  }

  handlePlayerInfo(playerInfo: playerInfoPackage) {
    console.log("come in director_handleDataFromSeverModel");
    VDEventListener.dispatchEvent(GAME_EVENT_DEFINE.SEND_PLAYER_INFO_TO_SCREENS_CONTROLER, playerInfo);
